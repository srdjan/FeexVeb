...
export function component(options) {
  ...
  class FeexComponent extends HTMLElement {
    constructor() {
      super();
      // ensure shadowRoot is explicit null when not using Shadow DOM
    ...
    getAttributeValue(key, defaultValue) {
      // derive default attribute name from state key (camelCase -> dash-case)
      const defaultAttr = key.replace(
        /[A-Z]/g,
        (letter) => `-${letter.toLowerCase()}`,
      );
      // allow custom attribute mapping (e.g., 'initial-value' for key 'value')
      let attrName = defaultAttr;
      if (!(attrName in attrs)) {
        for (const customAttr of Object.keys(attrs)) {
          if (customAttr.endsWith(`-${key}`)) {
            attrName = customAttr;
            break;
          }
        }
    ...
    attributeChangedCallback(name, oldValue, newValue) {
      ...
      // determine which state key corresponds to this attribute change
      const stateKey = Object.keys(state || {}).find((key) => {
        // default mapping: camelCase key -> dash-case attribute
        const defaultAttr = key.replace(
          /[A-Z]/g,
          (letter) => `-${letter.toLowerCase()}`,
        );
        if (defaultAttr === name || key === name) {
          return true;
        }
        // custom attribute mapping (e.g., 'initial-value' -> 'value')
        if (attrs[name] && name.endsWith(`-${key}`)) {
          return true;
        }
        return false;
      });
    ...
    performRender() {
      try {
        ...
        // If render returns a string, update synchronously
        if (typeof vdom === "string") {
          target.innerHTML = vdom;
          return;
        }
        // Use mono-jsx native rendering for VDOM nodes
